...
import { Search, Filter, FileText, Calendar, User, ArrowLeft } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-blue-200">Loading research...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 py-16">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <h1 className="text-4xl font-bold text-white mb-4">Research Papers</h1>
          <p className="text-xl text-blue-100">Explore our latest research and publications</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {filteredResearch.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-blue-200/60">Try adjusting your search or filter criteria</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredResearch.map((item) => (
              <motion.div
                ...
                className="bg-black/40 border border-blue-500/20 rounded-lg overflow-hidden hover:border-blue-500/50 transition-all duration-300"
              >
                <Card className="bg-transparent border-0 h-full">
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between mb-2">
                      <CardTitle className="text-lg font-semibold text-blue-100 line-clamp-2">
                        {item.title}
                      </CardTitle>
                      {getStatusBadge(item.published, item.featured)}
                    </div>
                    <CardDescription className="text-blue-200/80 line-clamp-3">
                      {item.description}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
